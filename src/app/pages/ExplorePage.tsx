...
export const ExplorePage: React.FC = () => {
  ...
  const filteredActivities = activities.filter(activity => {
    ...
    if (filters.neighbourhood !== 'All' && activity.neighbourhood !== filters.neighbourhood) return false;
    if (filters.searchQuery && !activity.title.toLowerCase().includes(filters.searchQuery.toLowerCase())) return false;
    return true;
  });

  const handleActivitySelect = (activity: any) => {
    setLocalSelectedActivity(activity);
    setShowDetailPanel(true); // Show detail panel when pin is clicked
  };

  const handleCardHover = (activity: any) => {
    // Only highlight the activity on map, don't open detail panel
    setLocalSelectedActivity(activity);
  };

  const handleActivityClick = (activity: any) => {
    // For list view, clicking opens the detail panel
    setLocalSelectedActivity(activity);
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex">
      ...
      <div className="w-[420px] bg-white border-r flex flex-col">
        ...
        <div className="p-4 border-b space-y-3">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors"
          >
              ...
              >
                ...
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setFilters({ ...filters, neighbourhood: value })}
                  >
                    <SelectTrigger className="h-9 text-sm">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <Button
                  ...
                  size="sm"
                  className="w-full"
                  onClick={() => setFilters({ category: 'All', neighbourhood: 'All', date: 'All', searchQuery: '' })}
                >
            ...
            filteredActivities.map((activity) => (
              <div 
                ...
                className={`bg-white border rounded-xl overflow-hidden transition-all hover:shadow-md ${
                  localSelectedActivity?.id === activity.id ? 'ring-2 ring-[#5661f6] shadow-md' : ''
                }`}
              >
                {/* Image */}
      ...
      <div className="flex-1 overflow-hidden bg-[#f5f5f7]">
        {view === 'map' ? (
          <LeafletMap
            ...
            selectedActivity={localSelectedActivity}
                  ...
                  {filteredActivities.map((activity, index) => (
                    ...
                    >
                      <ActivityCard
                        ...
                        isSelected={localSelectedActivity?.id === activity.id}
                        isInterested={activity.interestedUsers.includes(currentUser?.id || '')}
      ...
      {showDetailPanel && localSelectedActivity && (
        <ActivityDetailPanel
          activity={localSelectedActivity}
          isInterested={localSelectedActivity.interestedUsers.includes(currentUser?.id || '')}
          onToggleInterest={() => {
            ...
            if (updated) setLocalSelectedActivity(updated);
          }}
