...
export const AppProvider = ({ children }) => {
    ...
    const register = useCallback((name, email, password, neighbourhood) => {
        setCurrentUser({
            id: `user-${Date.now()}`,
            name,
            email,
    ...
    const toggleInterest = useCallback((activityId) => {
        setCurrentUser(prevUser => {
            if (!prevUser)
                return prevUser;
    ...
    // Computed values
    const unreadMessageCount = useMemo(() => conversations.reduce((sum, conv) => sum + conv.unreadCount, 0), [conversations]);
